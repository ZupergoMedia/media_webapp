...
import { reviewOwner } from "@/server/services/admin-service";

/**
 * POST /api/admin/owners/:id/review
 *
 * Approves, rejects or suspends a media owner account. Suspension also pauses
 * that owner's live inventory — see reviewOwner.
 */
...
export async function POST(
  ...
  try {
    ...
    const result = await reviewOwner({
      ownerId: id,
    ...
    console.error("[api/admin/owners/review]", error);
    return NextResponse.json({ error: "Could not record the decision." }, { status: 500 });
  }
}
